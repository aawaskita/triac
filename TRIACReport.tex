...
\chapter{Penerapan}
\section{Pendahuluan}
TRIA \textit{Code} yang telah dijelaskan sebelumnya secara umum dapat dikelompokkan menjadi dua tugas utama, masing-masing adalah perhitungan di waktu irradiasi dan kecelakaan. Untuk perhitungan pertama, kita harus bisa mendapatkan nilai OPF (persamaan \ref{eq:opf1}), $T_B$ (persamaan \ref{eq:TB}), $DS$ persamaan \ref{eq:DS}) dan $\tau_i$ (persamaan \ref{eq:taui}). Penerapannya adalah seperti pada Listing \ref{triac.py}.

\vfill
\scriptsize
\lstinputlisting[language=python, numbers=left, numberstyle=\tiny, caption=triac.py, showstringspaces=false, label=triac.py]{../TRIAC/inputdata2a.py}
\normalsize
